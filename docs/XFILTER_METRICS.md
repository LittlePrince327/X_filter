...
현재 이력서·경력기술서·포트폴리오·Notion에서 사용하는 분류 모델 비교 기록은 다음과 같습니다.
...
- 최종 분류 모델: **KcBERT**
- 선정 근거: Accuracy와 F1에서 비교 모델 중 가장 높은 성능 기록
- 대체 문장 생성: **RNN·LSTM, BLEU 73**

위 값은 2026년 2월과 7월에 작성된 이력서·경력기술서와 이전 포트폴리오의 모델 비교표에 반복 기록된 프로젝트 결과를 기준으로 정리한 값입니다. 현재 저장소에는 최종 수치를 텍스트로 저장한 단일 원시 평가 파일이 남아 있지 않으므로, 아래 재평가 절차를 통해 동일 평가 데이터 기준으로 다시 검증할 수 있습니다.
